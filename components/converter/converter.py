...
SCENARIOS = {
    ...
    "spoof": {
        "input_bin": DATA_DIR / "SS-18 GPS Static No Multipath True Position – 2 Spoofers.bin",
...
def extract_window_features(x: np.ndarray, window_start_sample: int, fs: int, label: int, spoofed_prn: str) -> dict | None:
...
def convert_tuni2025_to_feature_csv(
    ...
    print(f"Label: {label}")
    print(f"Spoofed PRN: {spoofed_prn}")
    ...
    with open(output_csv, "w", newline="") as f:
        ...
        while start_sample < total_samples:
            ...
            for w in range(full_windows_in_chunk):
                ...
                row = extract_window_features(x=x_window, window_start_sample=window_start_sample, fs=fs, label=label, spoofed_prn=spoofed_prn)
                if row is None:
                    skipped_windows += 1
                    continue
                rows.append(row)
...
def convert_clear_sky(max_seconds: float | None = None) -> pd.DataFrame:
    cfg = SCENARIOS["clear"]
    return convert_tuni2025_to_feature_csv(
        input_bin=cfg["input_bin"],
        output_csv=cfg["output_csv"],
        label=cfg["label"],
        spoofed_prn=cfg["spoofed_prn"],
        max_seconds=max_seconds,
    )

def convert_spoof(max_seconds: float | None = None) -> pd.DataFrame:
    cfg = SCENARIOS["spoof"]
    return convert_tuni2025_to_feature_csv(
        input_bin=cfg["input_bin"],
        output_csv=cfg["output_csv"],
...
def convert_both(max_seconds: float | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def run_default_converter() -> tuple[pd.DataFrame, pd.DataFrame]:
    return convert_both(max_seconds=None)
